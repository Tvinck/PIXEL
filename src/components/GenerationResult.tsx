...
const GenerationResult: React.FC<GenerationResultProps> = ({ result, type = 'image', onClose, onRemix }: GenerationResultProps) => {
    ...
    const handleDownload = async () => {
        ...
        if ((window as any).Telegram?.WebApp?.downloadFile) {
            try {
                (window as any).Telegram.WebApp.downloadFile({ url: result.url, file_name: `pixel-${Date.now()}.${type === 'video' ? 'mp4' : 'png'}` });
                return;
            } catch (e) {
    ...
    return (
        ...
        >
            ...
            <motion.div
                ...
                transition={{ delay: 0.5, type: "spring" as const, damping: 25 }}
