 ...
 */
export const SpringCard = ({ children, onClick, className = '' }: any) => {
 ...
 */
export const DraggableCard = ({ children, onDismiss, className = '' }: any) => {
    const [{ x, y }, api] = useSpring(() => ({ x: 0, y: 0 }));

    const bind: any = useDrag(({ down, movement: [mx, my] }: any) => {
        api.start({
            x: down ? mx : 0,
            y: down ? my : 0,
            immediate: down,
 ...
 */
export const SpringCounter = ({ value, className = '' }: any) => {
    ...
    return (
        <animated.span className={className}>
            {number.to((n: any) => Math.floor(n).toLocaleString())}
        </animated.span>
    );
};
 ...
 */
export const SpringProgress = ({ progress, className = '' }: any) => {
    const springProps = useSpring({
        width: `${progress}%`,
        config: config.slow
    });
 ...
 */
export const PulsingDot = ({ color = '#3390ec', size = 12, className = '' }: any) => {
    const springProps = useSpring({
 ...
 */
export const ShakeElement = ({ trigger, children, className = '' }: any) => {
    const springProps = useSpring({
        from: { x: 0 },
        to: trigger
            ? [
 ...
 */
export const SpringAccordion = ({ isOpen, children, className = '' }: any) => {
    const springProps = useSpring({
 ...
 */
export const FlipCard = ({ front, back, className = '' }: any) => {
 ...
 */
export const SlideNotification = ({ isVisible, children, className = '' }: any) => {
    const springProps = useSpring({
 ...
 */
export const MorphingBlob = ({ className = '' }: any) => {
    const springProps = useSpring({
 ...
 */
export const ParallaxScroll = ({ children, speed = 0.5, className = '' }: any) => {
