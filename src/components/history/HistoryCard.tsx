...
const HistoryCard = ({ gen, index, isSelected, isSelectionMode, handleCardClick }: any) => {
    return (
        <motion.div
            layoutId={`card-${gen.id}`}
            initial={{ opacity: 0, scale: 0.85, y: 20 }}
            animate={{ opacity: 1, scale: 1, y: 0 }}
            transition={{ delay: Math.min(index * 0.06, 0.6), type: 'spring' as const, stiffness: 260, damping: 22 }}
            whileTap={{ scale: 0.95 }}
