...
const StarCard: React.FC<any> = ({ star, isSelected, onSelect, index }) => (
    <motion.button
...
const OccasionCard: React.FC<any> = ({ occasion, isSelected, onSelect }) => (
    <motion.button
        whileTap={{ scale: 0.94 }}
...
const StarGreetingsView = () => {
    ...
    const [selectedOccasion, setSelectedOccasion] = useState<any>(OCCASIONS[0]);
    ...
    const telegramId = user?.telegram_id || ((window as any).Telegram?.WebApp?.initDataUnsafe?.user?.id);
