...
const StickersView = () => {
    ...
    const handleUpload = (e: any) => {
        ...
        if (file) {
            ...
            reader.onload = (ev: any) => { setSourceImage(ev.target.result); playClick(); };
                ...
                try {
                    const response = await fetch('/api/generate-stickers', {
                        ...
                        body: JSON.stringify({
                            ...
                            videoUrl: (sticker as any).videoUrl,
                    ...
                    if (data.success && data.imageUrl) {
                        newResults[i] = { ...sticker, status: 'success', resultUrl: data.imageUrl } as any;
                    } else {
    ...
    const downloadSticker = (url: string, title: string) => {
    ...
    const [sentToast, setSentToast] = useState<any>(null);
                            ...
                            <div className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2">
                                {selectedPack.stickers.map(sticker => {
                                    ...
                                    return (
                                        ...
                                        >
                                            {(sticker as any).videoUrl ? (
                                                <video src={(sticker as any).videoUrl} className="w-16 h-16 object-contain" autoPlay loop muted playsInline />
                                            ) : (
